...
<body>
    ...
    <div class="header-section">
        <!-- Header -->
        <h1 class="title-header">IPMI CLIENTS 2024</h1>
    </div>    
        ...
        <table id="table" class="table table-striped nowrap" style="width: 100%">
            <thead>
                <tr>
                    ...
                    <th>FULL NAME</th>
                    <th>
                        <select class="policy_filter" id="policy_filter" onchange="reloadTable();">
                    ...
                    <th>
                        <select class="insurer_filter" id="insurer_filter" onchange="reloadTable();">
                    ...
                    <th>
                        <select class="source_inquiry" id="source_inquiry" onchange="reloadTable();">
                    ...
                    <th>
                        <select class="lives_2024_filter" id="lives_2024_filter" onchange="reloadTable();">
                            <option value="">Lives</option>
                            @foreach ($Lives_2024_filter as $lives_2024)
                            <option value="{{ $lives_2024 }}">{{ $lives_2024 }}</option>
                            @endforeach
                        </select>
                    </th>
                    <th>
                        <select class="group_name" id="group_name" onchange="reloadTable();">
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            const table = $('#table').DataTable({
                ...
                ajax: {
                    ...
                    data: function (d) {
                        ...
                        d.lives_2024 = $('#lives_2024_filter').val();
                ...
                columns: [
                    ...
                    { data: 'lives_2024', name: 'lives_2024', orderable: false },
                    ...
                    { data: 'case_closed', name: 'case_closed', orderable: false },
                    {
                        ...
                        render: function (data, type, row) {
                            var actions = '<a href="' + "{{ url('/2024/accountsection/') }}/" + row.id + '/" title="Show Client"><button class="actionBtn"><i class="fa fa-eye"></i></button></a>';
                            if (userRole === 'IT admin' || userRole === 'Encoder') { // Allow both roles 1 and 2
                                actions += '<a href="' + "{{ url('/2024/accountsection/') }}/" + row.id + '/edit" title="Edit Client"><button class="actionBtn"><i class="fa-solid fa-square-pen"></i></button></a>' +
                                        '<button class="actionBtn delete-btn" data-id="' + row.id + '" title="Delete Client"><i class="fa-solid fa-circle-minus"></button>';
                            }
                            return actions;
                        }
                    }
            ...
            function updateFilterOptions(filters) {
                ...
                updateDropdownOptions('#lives_2024_filter', filters.lives_2024, 'Lives');
            ...
            $.ajax({
                type: 'POST',
                url: "{{ url('/2024/accountsection/delete') }}/" + id,
                headers: {
            ...
            function transferData(id) {
                $.ajax({
                    url: '/2024/accountsection/' + id + '/transfer',
                    method: 'POST',
            ...
            function updateExportCsvUrl() {
                var filters = {
                    ...
                    lives_2024: $('#lives_2024_filter').val(),
                    group_name: $('#group_name').val(),
                };

                var queryString = $.param(filters);
                var newExportCsvUrl = "{{ route('export_2024.csv') }}" + "?" + queryString;
                $("a.export-btn").attr("href", newExportCsvUrl);
            }

            $('.source_inquiry, .policy_filter, .insurer_filter, .lives_2024_filter, .group_name').on('change', function () {
                updateExportCsvUrl();
                reloadTable();
            });
