        ...
        <div class="card">
            <table id='myTable10' class="table">
                ...
                <tbody>
                    ...
                    $case_officer_2023 = array();
                    $query = "SELECT DISTINCT case_officer_2023 FROM accountsection_2023 
                              WHERE insurance_type ='IPMI' 
                              AND (status = 'Complete' OR status = 'Incomplete')
                              AND (lives_2023='Existing' OR lives_2023='New Lives And Existing')   
                              AND ((policy='Active' AND start_date BETWEEN '2023-01-01' AND '2023-12-31') 
                              OR (policy='Lapsed' AND DATEDIFF(cancelled_date, start_date) >= 180))
                              AND start_date BETWEEN '2023-01-01' AND '2023-12-31'
                              ORDER BY case_officer_2023";

                    $query_run = mysqli_query($con, $query);
                    if (mysqli_num_rows($query_run) > 0) {
                        while ($row = mysqli_fetch_assoc($query_run)) {
                            $case_officer_2023[] = $row['case_officer_2023'];
                        }
                    }

                    $total_counts = array_fill(1, 12, 0);
                    $grand_total = 0;
                    foreach ($case_officer_2023 as $case_officer) {
                        $case_total = 0;
                        $counts = array_fill(1, 12, 0);

                        $query = "SELECT MONTH(start_date) as month, COUNT(case_officer_2023) as count 
                                  FROM accountsection_2023
                                  WHERE case_officer_2023 = '$case_officer' 
                                  AND (status = 'Complete' OR status = 'Incomplete')
                                  AND (lives_2023='Existing' OR lives_2023='New Lives And Existing')   
