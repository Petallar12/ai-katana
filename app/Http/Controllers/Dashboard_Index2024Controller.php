...
class Dashboard_Index2024Controller extends Controller
{
    ...
    {  
        ...
        return view('2024.dashboard_index.source_index', compact('source_inquiry'));
    }
    ...
    {  
        ...
        return view('2024.dashboard_index.insurer_index', compact('insurer'));
    }
    ...
    {  
        ...
        return view('2024.dashboard_index.country_index', compact('country_residence'));
    }
    ...
    {  
        ...
        return view('2024.dashboard_index.age_index', compact('age'));
    }
    ...
    {  
        ...
        return view('2024.dashboard_index.country_premium_index', compact('country_residence'));
    }
    ...
    public function insurer_premium_index(Request $request)
    {
        ...
        if ($request->ajax()) {
            ...
            $query = Accountsection_2024::select('*') // Adjust the select columns as needed
            ->where('status', '!=', 'Not Counted') 
            ->where('insurance_type','IPMI')
            ->whereNotIn('policy', ['Transferred', 'Pended'])
            ->whereBetween('start_date',['2024-01-01' , '2024-12-31'])
        ...
        return view('2024.dashboard_index.insurer_premium_index', compact('insurer'));
    }

    public function insurer_commission_index(Request $request)
    {
        // Get distinct source inquiries for the dropdown
        $insurer = Accountsection_2024::query()
            ->distinct()
        ...
        if ($request->ajax()) {
            ...
            ->whereNotIn('policy', ['Transferred', 'Pended'])
            ->whereBetween('start_date',['2024-01-01' , '2024-12-31'])
            ->whereNotNull('commission_2024') // Exclude rows where convert_premium_USD is null
            ->where('commission_2024', '!=', ''); // Exclude rows where convert_premium_USD is blank
        ...
        // Pass the distinct source inquiries to the view for the dropdown
        return view('2024.dashboard_index.insurer_commission_index', compact('insurer'));
    }
    ...
    //use in Query Parameter
        ...
        return view('2024.dashboard_index.case_closed_inquiry_index', compact('caseOfficer'));
    }
    
    public function newlives_2024_index(Request $request)
    {
        $placement_date = $request->query('placement_date');
        if ($request->ajax()) {
            $query = Accountsection_2024::select('*')
            ->whereIn('policy',['Active' , 'Pended', 'Lapsed'])
            ->where('insurance_type' , 'IPMI')
            ...
            if ($placement_date) {
                ...
                $monthNumber = date('m', strtotime($placement_date . " 1 2024"));
        ...
        return view('2024.dashboard_index.newlives_2024_index', compact('placement_date'));
    }
    
    public function existing_2024_index(Request $request)
    {
        if ($request->ajax()) {
            ...
            if ($start_date) {
                $monthNumber = date('m', strtotime($start_date . " 1 2024"));
        ...
        return view('2024.dashboard_index.existing_2024_index');
    }
